    ...
    return (
        <div id={styles.container}>
            <ul id={styles.fileList}>
                {fileList.map((file, i) => {
                    const {name, imageLink, isImage, isFolder} = file;
                    return (
                        <a key={i} className={styles.fileLink} href={imageLink} rel="noreferrer noopener" target="_blank">
                            <li className={styles.file}>
                                { isImage ? 
                                    <img className={styles.img} src={imageLink} alt={name} title={name} /> : isFolder ?

                                    <svg className={styles.svg} alt={name} xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 512 512">
